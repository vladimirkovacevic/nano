...
data.ref <- readRDS("C:\\Users\\NenadPC\\Documents\\seurat_object.rds")
...
data.ref <- RunUMAP(data.ref, dims = 1:30)

sample <- Read10X(data.dir = "C:\\Users\\NenadPC\\Desktop\\RNA\\Uzorak_4_7\\uzorak")

data.query <- CreateSeuratObject(counts = sample, project = "uzorak_1_7", min.cells = 3, min.features = 200)
data.query
...
data.query <- RunUMAP(data.query, dims = 1:30)
...
data.query <- MapQuery(anchorset = anchors, reference = data.ref, query = data.query,
                           refdata = list(celltype = "CellType"), reference.reduction = "pca", reduction.model = "umap")

p1 <- DimPlot(data.ref, reduction = "umap", group.by = "CellType", label = TRUE, label.size = 3,
              repel = TRUE) + ggtitle("Reference annotations")
p2 <- DimPlot(data.query, reduction = "ref.umap", group.by = "predicted.id", label = TRUE,
...
table(data.query$predicted.id)
sink("uzorak4.csv")
as.matrix(data.query$'predicted.celltype')
sink()
